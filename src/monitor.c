...
/*   Updated: 2025/01/02 00:55:35 by eduribei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*stop_monitor(void *arg)
{
	...
	while (1)
	{
		...
		while (i < r->params->nb_philos)
		{
			...
			t_since_last_meal = ft_t_delta_us(r->philo[i].t_of_last_meal, curr_time);
			pthread_mutex_lock(&r->stop_mutex);
			if (t_since_last_meal >= r->params->time_to_die)
			{
				r->params->must_stop = true;
				timedelta = ft_t_delta_ms(r->params->start_time, curr_time);
