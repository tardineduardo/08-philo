...
/*   Updated: 2025/01/02 00:47:52 by eduribei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool    philo_eating(t_philos *philo, t_params *params)
{
    ...
    should_stop = (philo->number_of_meals_had == params->number_of_meals_to_eat);
    ...
    if (should_stop)
    {
        ...
        printf(BLUE "--- %li had all %li meals! \n" RESET, 
               philo->index + 1, params->number_of_meals_to_eat);
        pthread_mutex_unlock(params->print_mutex);
        return (false);
    }
    return (true);
...
bool    philo_thinking(t_philos *philo, t_params *params)
{
    ...
    if (philo->index % 2 == 0)
        usleep(1000);  // 1ms for even philosophers
    else
        usleep(500);   // 0.5ms for odd philosophers
